...
use boss\entity\monster\PigZombie;
...
class Main extends PluginBase implements Listener
{
    ...
    public function onEnable()
    {
        ...
        Entity::registerEntity(PigZombie::class, true);
        ...
        if (strtolower($cmd->getName()) == "boss") {
            ...
            switch (strtolower($args[0])) {
                ...
                case "inv":
                    ...
                    $sender->getInventory()->addItem(Item::get(383, 36)->setCustomName("§r§a§lBOSS§r§a - Beatifull\n§7Clique no chão para spawna"));
                    break;
    ...
    public function onBreak(BlockBreakEvent $e)
    {
        $player = $e->getPlayer();
        if (mt_rand(1, 150) == 1) {
            $player->getInventory()->addItem(Item::get(383, 36)->setCustomName("§r§a§lBOSS§r§a - Beatifull\n§7Clique no chão para spawna"));
    ...
    public function onTap(PlayerInteractEvent $e)
    {
        ...
        $y = $b->getY() + 1.5;
        $z = $b->getZ() + 0.5;
        if ($item->getId() == 383 && $item->getDamage() == 36 && $item->getCustomName() == "§r§a§lBOSS§r§a - Beatifull\n§7Clique no chão para spawna") {
            $e->setCancelled(true);
            $nbt = new CompoundTag("", [
            new ListTag("Pos", [
                new DoubleTag("", $x),
            ...
            $ent = Entity::createEntity("PigZombie", $player->getLevel(), $nbt);
            $ent->spawnToAll();
            $ent->setMaxHealth(2500);
            $ent->setHealth(2500);
